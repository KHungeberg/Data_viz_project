...
output:
  ...
  pdf_document:
    toc: true
    toc_depth: 3
github_repo: https://github.com/KHungeberg/Data_viz_project
---
...
Simple note: Pearson correlation measures the strength of the linear relationship. 
...
numdat %>% 
  ...
  labs(title = "Heatmap of numerical values pearson correlation", x = "Column", y = "Row", fill = "correlation") +
  theme_minimal() +
...
Simple note: Spearman correlation measures the monoteny of between variables
```{r,fig.height=10,fig.align='center'}
numdat %>% 
  cor(method = "spearman") %>% 
  as.tibble() %>% 
  ...
  geom_text(aes(label = round(correlation, 2)), color = "black", size = 3) +  # Add text on each tile
  scale_fill_gradientn(colors = c("blue", "white", "red")) + 
  labs(title = "Heatmap of numerical values spearman correlation", x = "Column", y = "Row", fill = "correlation") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
--------- NOTE ALL NA-VALUES ARE OMITTED ------ 
...
A non linear approach to dimensional reduction. 
...
datapiv %>% 
  ...
  ggplot(aes(bill_depth_mm,flipper_length_mm,color=value)) +
  geom_point()
...
### Scatterplots colored by category

This should essentially have every combination of every combination of the numerical features with the categorical. 
```{r,fig.show='hold',out.width="50%",results='hide',warning=FALSE,message=FALSE}
catnumnum_combs <- expand_grid(categorical_features,numerical_features,numerical_features)
catnumnum_combs <- catnumnum_combs %>% 
  rename("numerical_features_1" = numerical_features...2,
         "numerical_features_2" = numerical_features...3) %>% 
  filter(numerical_features_1!=numerical_features_2)

catnumnum_combs %>% 
  mutate(get_data = 
           pmap(list(categorical_features,numerical_features_1,numerical_features_2),
                ~(
                  datapiv %>% 
                    filter(feature==..1) %>% 
                    select(..2,..3,value) %>% 
                    rename(all_of(c(col1 = ..2))) %>%
                    rename(all_of(c(col2 = ..3))) %>% 
                    ggplot(aes(col1,col2,color=value)) +
                    geom_point() +
                    xlab(..2) +
                    ylab(..3) + 
                    ggtitle(paste("Scatterplot of",..2,"and",..3,"coloured by",..1,sep = " "))
                ))) %>% 
  pull(get_data)
```
  ...
  mutate(
    boxplots = map(data,~(
      .x %>% 
        ...
        facet_wrap(~numerical_features,scales = "free") +
        ggtitle(paste("Violin plot of numerical features, faceted by",categorical_features))
    ))
  ) %>% pull(boxplots)
...
### Bar plot of mean with standard deviations 
    ...
    get_data = map2(
      categorical_features,numerical_features,~(
        datapiv %>% 
          ...
          group_by(value) %>% 
          summarise(
            avg = mean(col1,na.rm = T),
            std = sd(col1,na.rm = T)
          )))) %>% 
  mutate(
    plots = pmap(list(categorical_features,get_data,numerical_features),~(
      ..2 %>% 
        ggplot(aes(value,avg,fill=value)) +
        geom_bar(stat="identity") +
        geom_errorbar(aes(ymin = avg-std,ymax = avg+std),width = 0.25) +
        labs(x="",y=paste("mean of",..3,sep=" "))+
        ggtitle(paste("Mean bar chart of",..3,"by",..1,sep = " "))
    ))) %>% pull(plots)
